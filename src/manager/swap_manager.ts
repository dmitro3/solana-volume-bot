...
export const transferSOL = async (
  ...
  const result = await doTransferSOL(
    from,
    to,
    amount,
    async (msg: string) => {
...
export const calcFee = (
  ...
  const swapOrgFee = (amount * global.Swap_Fee_Percent) / 100.0;
  ...
  if (rewardAvailable && session.referredBy) {
    ...
    if (session.referredTimestamp) {
      monthSpent =
        (now - session.referredTimestamp) / (1000 * 60 * 60 * 24 * 30);
    }
...
export const doTransferSOL = async (
  pkey: string,
  destWallet: string,
  amount: number,
  sendMsg: Function,
  ...
  const messageV0 = new TransactionMessage({
    ...
    recentBlockhash: (await global.web3Conn.getLatestBlockhash()).blockhash,
...
export const getBuyTokenTrx = async (payer: any, token: any, amount: any) => {
  ...
  try {
    ...
    const walletTokenAccounts = await utils.getWalletTokenAccount(
    ...
    return {
      trxs: transactions,
      amount: amountOut.toFixed(baseToken.decimals),
    };
  } catch (error) {
    console.log(error);
  }
...
export const getSellTokenTrx = async (payer: any, token: any, amount: any) => {
  ...
  try {
    ...
    return {
      trxs: transactions,
      amount: amountOut.toFixed(quoteToken.decimals),
    };
  } catch (error) {
    console.log(error);
  }
