...
const getAPIKeyIndex = () => {
  const index = Math.floor(
    (constants.JITO_AUTH_KEYS.length - 1) * Math.random()
  );
...
export const createAndSendBundleTransaction = async (
  ...
  const wallet = utils.getWalletFromPrivateKey(
    constants.JITO_AUTH_KEYS[getAPIKeyIndex()]
  );
  const searcher = searcherClient(global.get_jito_block_api(), wallet.wallet);
  ...
  try {
    const recentBlockhash = (
      await global.web3Conn.getLatestBlockhash("finalized")
    ).blockhash;
