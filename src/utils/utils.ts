...
import * as crypto from "../utils/cutils";
...
export const getTokenInfo = async (addr: string) => {
  const metaplex = Metaplex.make(global.web3Conn);
  ...
  const metadataAccountInfo = await global.web3Conn.getAccountInfo(mintAddress);
  ...
  if (metadataAccountInfo) {
    ...
    if (token) {
      console.log("________token_______1", token);
      return {
        exist: true,
      ...
      console.log("________token_______2");
    ...
    if (token) {
      console.log("________token_______3", token);
      return {
        exist: true,
        symbol: token.mint.currency.symbol,
        decimal: token.mint.currency.decimals,
      ...
      console.log("________token_______4");
      return { exist: false, symbol: "", decimal: 0 };
    }
  }
};
...
export const IsTokenAccountInWallet = async (wallet: any, addr: string) => {
  const walletTokenAccount = await global.web3Conn.getTokenAccountsByOwner(
...
export const getWalletSOLBalance = async (wallet: any): Promise<number> => {
  ...
  try {
    let balance: number =
      (await global.web3Conn.getBalance(new PublicKey(wallet.publicKey))) /
      LAMPORTS_PER_SOL;
    return balance;
  } catch (error) {
    console.log(error);
